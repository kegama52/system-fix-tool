...
namespace TreasuryAIChat.Services
{
    ...
    {
        Task<IReadOnlyList<KBResult>> SearchAsync(string query, int topK = 3, System.Threading.CancellationToken ct = default);
    }
    ...
    public class TreasuryKnowledgeBaseService : IKnowledgeBaseService
    {
        ...
        public Task<IReadOnlyList<KBResult>> SearchAsync(string query, int topK = 3, System.Threading.CancellationToken ct = default)
        {
            using var db = _dbFactory.CreateDbContext(ct);
            var entries = db.KnowledgeBases.Where(k => k.IsActive).AsNoTracking().ToList(ct);
